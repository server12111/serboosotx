...
- На проде хостинг сам собирает и запускает контейнер (своим билдпаком) — в репо нет
  своего Dockerfile/docker-compose.yml, хостинг требует `DATA_DIR=/app/data` (эта
  папка на нём сохраняется между перезапусками — там же лежит БД, см. ниже).
